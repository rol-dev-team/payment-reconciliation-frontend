...
export default function ServiceUploadSection({ values, onUpload, onDelete }) {
  ...
  // Load and filter wallets by selected channel
  useEffect(() => {
    if (!values.channel) {
      setWalletOptions([]);
      return;
  ...
  const handleFileChange = (files) => {
    const selectedChannelLabel = channelOptions.find((c) => c.id === values.channel)?.label || "N/A";
    const selectedWalletLabel = walletOptions.find((w) => w.id === values.wallet)?.label || "N/A";
    ...
    files.forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        setTempFiles((prev) =>
          prev.map((f) => (f.id === id ? { ...f, transactions: transactionCount } : f))
        );
      };

      setTempFiles((prev) => [...prev, tempFile]);
      onUpload(file, { channel: values.channel, wallet: values.wallet }, id);
        ...
        >
          <Box sx={{ flex: 1 }}>
            <SelectDropdownSingle
              ...
              onChange={values.setFieldValue ? (val) => values.setFieldValue("channel", val) : undefined}
              height={42}
            />
          </Box>
          ...
          <Box sx={{ flex: 1 }}>
            <SelectDropdownSingle
              ...
              fetchOptions={async () => walletOptions}
              value={values.wallet}
              onChange={values.setFieldValue ? (val) => values.setFieldValue("wallet", val) : undefined}
              height={42}
              disabled={!values.channel}
            />
          </Box>
