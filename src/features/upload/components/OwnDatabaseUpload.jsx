...
export default function OwnDatabaseUpload({ values, onUpload, onDelete }) {
  ...
  const handleFilesChange = (files) => {
    ...
    files.forEach((file) => {
      ...
      setFilesList((prev) => [...prev, tempFile]);
      onUpload(file, { billingSystemId: values.billingSystem }, id);
        ...
        >
          <Box sx={{ flex: 1 }}>
            <SelectDropdownSingle
              ...
              placeholder="Select Billing System"
              fetchOptions={async () => billingOptions}
              value={values.billingSystem}
              onChange={values.setFieldValue ? (val) => values.setFieldValue("billingSystem", val) : undefined}
              height={42}
            />
          </Box>
