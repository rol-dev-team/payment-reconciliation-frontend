...
import React, { useState, useRef } from "react";
import {
  Box, TextField, Paper, Popper, Typography, ClickAwayListener, Button,
} from "@mui/material";
import CachedIcon from "@mui/icons-material/Cached";
import { Formik } from "formik";
import { format } from "date-fns";
...
export default function UploadForm() {
  ...
  const handleCompare = async () => {
    if (serviceFiles.length === 0 || ownFiles.length === 0) return;

    try {
      setIsSubmitting(true);
      const result = await submitReconciliation(serviceFiles, ownFiles, appliedRange);
      console.log("Reconciliation result:", result);
      alert(`Files submitted successfully! Batch ID: ${result.batch_id}`);
